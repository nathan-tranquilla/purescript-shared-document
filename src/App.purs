...
type State = { input :: Maybe String, pos :: Maybe Int, version :: Int }

data Action = Input String (Maybe EventTarget)
...
showState state = foldl (\acc txt -> acc <> " " <> txt) ""
  $ filter (\str -> length str > 0)
      [ showText "Input" state.input
      , showPos state.pos
      , "Version: " <> show state.version
      ]
